...
- Every record carries a sealantd-minted `EventId` (non-empty string compatible with `sandboxEventSchema.eventId`, integration brief §2/§8) and an execution `Sequence`. The `EventId` is the idempotency key used by the SDK to drop duplicates from at-least-once resend (plan §15 "Event IDs and idempotent delivery keys", "Duplicate-safe resend").
