...
- `workspaceId` — `text` PK, opaque/UUID-like; SSH carries it as `ws-{id}` in the
  username (`apps/ssh-gateway/src/workspace-target.ts`,
  `packages/db/src/schema/control-plane.ts` workspaces table).
- `attemptId` / `runId` — **the same value**, the per-run correlation key;
  `workspace_runtime_instances.run_id` is its PK 1:1
  (`packages/db/src/schema/workspace-build-jobs.ts:23,67`;
  `control-plane.ts` workspaceAttempts).

There is **no process-, PTY-, session-, or event-ID format yet** (integration
brief §2). The only event-ID precedent is
`workspaceEventSchema.eventId: NonEmptyTrimmedString`
(`packages/api-contracts/src/core-api/workspaces.ts:195`) — a bare non-empty
string with no prefix convention (`ws_`/`run_` appear only in tests). The OS PID
...
- `RuntimeId` identifies the **daemon instance** — one per workspace+run. It is the
...
- `ExecutionId` **carries the monorepo `runId` (== `attemptId`)**, the per-run
  ...
  sealantd event joins back to `workspace_runtime_instances.run_id`
  (`workspace-build-jobs.ts:67`) and `workspace_attempts`.
- `workspaceId` is **bound at config level** (plan §9 identity/execution config),
  not minted by sealantd — it is the daemon's static run scope (integration
  brief §1 binds the socket per workspace; §2 requires every event carry
  `workspaceId`).
- **sealantd mints `SessionId`, `ProcessId`, and `EventId`.** No pre-existing
  ...
  compatible with `workspaceEventSchema.eventId`
  (`api-contracts/.../workspaces.ts:195`) and bind every event to
  `runId`(=`ExecutionId`) + `workspaceId` (integration brief §2, requirement 8).
...
- Every event self-describes its correlation: `runtimeId`, `executionId`
  (=`runId`/`attemptId`), `workspaceId`, plus minted `sessionId`/`processId`,
  so the trail joins cleanly onto `workspace_runtime_instances` and the
  `workspaceEventSchema` envelope (integration brief §2, §4).
...
- `sequence` ordering is meaningful only within a `RuntimeId`; cross-runtime or
  cross-workspace global ordering is not provided and consumers must not assume it.
...
- **Reuse the monorepo `eventId` generator or adopt a `ws_`/`run_` prefix
